...
Require Import gem5.
Require Import gem5fixed.
Require Import opensparc.
Require Import risc.
Require Import scproc.
Require Import ppo.
Require Import litmus.

Definition AllProcessors :=
  [SCProc_Pipeline 4; RISC_Pipeline 4; gem5_O3_Pipeline 4; gem5fixed_O3_Pipeline 4; OpenSPARC_T2_Pipeline 4].

Definition AllPPOGraphs := map GraphsToVerifyTSOPPO AllProcessors.

Definition AllLitmusTestGraphs :=
    map (fun p => (fst p) (snd p))
      (CartesianProductPairs AllLitmusTests AllProcessors).

Definition AllGraphs :=
  fold_left (app (A:=_))
    (AllPPOGraphs ++ AllLitmusTestGraphs)
    [].
...
*)
Extraction "allgraphs.ml" AllGraphs.
